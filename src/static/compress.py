...
def compress_img(image_name, new_size_ratio=0.9, quality=90, width=None, height=None, to_jpg=True):
    ...
    if to_jpg:
        ...
        if os.path.isdir(f"{args.to_dir}/{sub_dir}/"):
          new_filename = f"{args.to_dir}/{sub_dir}/{filename}.jpg"
        else:
          os.mkdir(f"{args.to_dir}/{sub_dir}/")
          new_filename = f"{args.to_dir}/{sub_dir}/{filename}.jpg"
    else:
        # retain the same extension of the original image
        # new_filename = f"{filename}_compressed{ext}"
        if os.path.isdir(f"{args.to_dir}/{sub_dir}/"):
          new_filename = f"{args.to_dir}/{sub_dir}/{filename}{ext}"
        else:
          os.mkdir(f"{args.to_dir}/{sub_dir}/")
          new_filename = f"{args.to_dir}/{sub_dir}/{filename}{ext}"
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("from_dir", help="directory of the original images")
    parser.add_argument("to_dir", help="directory of the compressed images")
    ...
    # compress the image
    print(f'from "{args.from_dir}"')
    print(f'to "{args.to_dir}"')

    sub_folders = glob.glob(f'{args.from_dir}/*')
    # files = glob.glob(f'{args.from_dir}/*')
    for sub_folder in sub_folders:
        ...
        for file in files:
            ...
            print("="*50)
            compress_img(file, args.resize_ratio, args.quality, args.width, args.height, args.to_jpg)
            # break
